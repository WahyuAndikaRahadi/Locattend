...
export default function AdminOfficesEdit({ office }) {
    const { data, setData, put, processing, errors } = useForm({
        ...
        radius_meters: office.radius_meters || 200,
        clock_in_time: office.work_schedule?.clock_in_time?.slice(0, 5) ?? '08:00',
        clock_out_time: office.work_schedule?.clock_out_time?.slice(0, 5) ?? '17:00',
        working_days: office.working_days || [1, 2, 3, 4, 5, 6],
    });

    const [mapCenter, setMapCenter] = useState([
        parseFloat(office.latitude) || -6.1751, 
        parseFloat(office.longitude) || 106.8650
    ]);
    ...
    return (
        <AuthenticatedLayout header="Edit Konfigurasi Kantor">
            ...
            <div className="max-w-6xl mx-auto pb-12 animate-slide-up px-4 sm:px-6">
                <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-5">
                        ...
                        <div>
                            <h2 className="text-3xl font-black text-slate-900 tracking-tight">{office.name}</h2>
                            <p className="text-slate-500 font-medium mt-0.5">Sesuaikan titik koordinat dan radius geofencing secara visual.</p>
                        </div>
                    </div>
                    <Link 
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                                ...
                                <div className="absolute bottom-10 left-10 right-10 z-[1000]">
                                    <div className="bg-white/90 backdrop-blur-md p-6 rounded-3xl border border-white shadow-2xl flex items-center gap-5">
                                        ...
                                        <p className="text-[11px] text-slate-600 font-bold leading-relaxed">
                                            Geser marker biru pada peta atau klik dimanapun untuk memindahkan titik koordinat kantor.
                                        </p>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="pt-4 flex flex-col sm:flex-row gap-4">
                        <Link
                            href={route('admin.offices.index')}
                            className="flex-1 py-4 bg-white border-2 border-slate-100 text-slate-400 font-black rounded-2xl transition-all hover:bg-slate-50 hover:text-slate-600 flex items-center justify-center gap-2 active:scale-95 text-center"
                        >
                            Batal Edit
                        </Link>
                        <button 
                            type="submit" 
                            disabled={processing} 
                            className="flex-[2] py-4 bg-primary-600 hover:bg-primary-700 text-white font-black rounded-2xl transition-all shadow-xl shadow-primary-500/20 text-base flex items-center justify-center gap-2 active:scale-95 disabled:opacity-50"
                        >
                            <svg className="w-5 h-5 font-black" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
