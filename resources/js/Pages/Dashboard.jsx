...
const StatusBadge = ({ status }) => {
    ...
    return (
        <span className={`inline-flex px-3 py-1 text-[10px] font-black uppercase tracking-wider rounded-full ${styles[status] || 'bg-slate-100 text-slate-600'}`}>
        ...
        <div className="space-y-8 animate-slide-up">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                <StatCard 
                    ...
                    value={todayAttendance ? labels[todayAttendance.status] : 'Belum Absen'}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                {office && (
                    <div className="bg-gradient-to-br from-slate-900 to-slate-800 p-8 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden group">
                        <div className="absolute top-0 right-0 w-64 h-64 bg-blue-500 opacity-10 rounded-full -mr-20 -mt-20 blur-3xl group-hover:scale-150 transition-transform duration-1000"></div>
                        <div className="relative z-10">
                            <p className="text-blue-400 font-black text-[10px] uppercase tracking-widest mb-4">Lokasi Kantor Anda</p>
                            <h3 className="text-3xl font-black mb-2">{office.name}</h3>
                            <div className="flex items-center gap-4 mt-6">
                                <div className="px-4 py-2 bg-white/10 rounded-xl text-sm font-bold backdrop-blur-md border border-white/10">
    ...
    return (
        <div className="space-y-8 animate-slide-up">
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-8">
                <Link href={route('supervisor.team')} className="flex-1 py-4 bg-slate-900 text-white rounded-2xl text-center font-bold hover:bg-slate-800 transition-all shadow-xl shadow-slate-200">
                    👥 Dashboard Tim
                </Link>
                <Link href={route('attendance.index')} className="flex-1 py-4 bg-white border border-slate-200 text-slate-900 rounded-2xl text-center font-bold hover:bg-slate-50 transition-all shadow-sm">
                    📍 Clock In
                </Link>
            </div>
        </div>
...
function AdminDashboard({ totalUsers, totalOffices, todayTotalAttendance, totalPendingLeaves, attendanceTrends, recentActivities, lateComersCount }) {
    return (
        <div className="space-y-10 animate-slide-up">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="space-y-8">
                        ...
                        <div className="space-y-5">
                            {recentActivities?.length > 0 ? recentActivities.map((activity) => (
                                <div key={activity.id} className="flex items-center gap-4 group cursor-pointer">
                                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 transition-transform group-hover:scale-110 ${activity.status === 'terlambat' ? 'bg-rose-50 text-rose-500' : 'bg-emerald-50 text-emerald-500'}`}>
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-[11px] font-bold text-slate-500 mt-0.5">{activity.time} &bull; <span className={activity.status === 'terlambat' ? 'text-rose-500' : 'text-emerald-500'}>{activity.status === 'terlambat' ? 'Terlambat' : 'Hadir'}</span></p>
                                    </div>
                                </div>
                            )) : (
                    ...
                    {/* Operational Alert - Warm Theme */}
                    <div className="bg-gradient-to-br from-amber-400 to-orange-600 p-8 rounded-[2.5rem] shadow-xl shadow-orange-200 relative overflow-hidden group">
                        <div className="absolute top-0 right-0 w-32 h-32 bg-white opacity-20 rounded-full -mr-10 -mt-10 blur-2xl group-hover:scale-150 transition-transform duration-700"></div>
                        <div className="relative z-10">
                            <div className="flex items-center gap-4 mb-6">
                                ...
                                <h3 className="text-xl font-black text-white tracking-tight">Kesiagaan</h3>
                            </div>
                            <div className="space-y-4">
                                <div className="flex items-center justify-between p-4 bg-black/10 rounded-2xl border border-white/10">
                                    <span className="text-sm font-bold text-orange-50">Terlambat Hari Ini</span>
                                    <span className="text-3xl font-black text-white">{lateComersCount || 0}</span>
                                </div>
                                <button className="w-full py-4 bg-white text-orange-600 text-xs font-black uppercase tracking-[0.2em] rounded-2xl transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5 active:scale-95">
                                    Beri Notifikasi
                                </button>
                            </div>
                        </div>
